...
void LHRStime()
{
  ...
  //Int_t runNUM = 90854;
  // Int_t evtNUM = 10000;
TH1F *h1=new TH1F("h1"," ",500,0.10e-6,0.15e-6);
 TString filename;
 for(int irun=100400;irun<100403;irun++) //100380;irun<100585
   {
			 ...
			 for (int i=1;i<15;i++)
			   {
			     if(LHRS_trig_bit2 && Ls2_nthit==1 &&  Ls2_pad[0]==i)
                            {
			      double RF_s2_mean = (Ls2_tdchit[37] -Ls2_tdchit[47])*tdcTime - (((Ls2_tdchit[30] - Ls2_tdchit[i])*tdcTime + (Ls2_tdchit[37] - Ls2_tdchit[i+48])*tdcTime )/2.0 + corr_x[i-1]*L_trx[0] + corr_th[i-1]*L_trth[0] + corr_adc[i-1]*Ls2_ladc[i] + alignment[i-1]);
...
TCanvas *c1 = new TCanvas("c1","c1",600,600);
 ...
 h1->Draw();
  ...
  h1->SetTitle("L-HRS S2 Timing Alignments");
  ...
  l1.DrawLatex(0.115e-6,1150,Form("#color[2]{#sigma #approx 250 ps}"));
 
  //  c1->SaveAs("LHRStime1.pdf"); 
}
