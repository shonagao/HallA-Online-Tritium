...
// Looking for an event either that pass throught #9 or #10 or both
...
void L_add10_11()
{
  ...
  //Int_t runNUM = 90854;
  // Int_t evtNUM = 10000;
TH1F *h11=new TH1F("h11","h11 with  +9.985e-10",500,0.10e-6,0.15e-6);

 TString filename;
 for(int irun=100400;irun<100419;irun++)
   {
...
RF_s2_pad10_meanCorr = (Ls2_tdchit[37] -Ls2_tdchit[47])*tdcTime  - (((Ls2_tdchit[30] - Ls2_tdchit[10])*tdcTime + (Ls2_tdchit[37] - Ls2_tdchit[58])*tdcTime )/2.0 + 8.74297e-09*L_trx[0] -3.39145e-08*L_trth[0] -1.3259e-12*Ls2_ladc[10] -7.9345e-10);
 ...
 // Defining cuts
	   if(LHRS_trig_bit2 && Ls2_pad[0]==10 && Ls2_nthit==1)
	   {
             RF_s2_mean = RF_s2_pad10_meanCorr;
	   }
	   else if( LHRS_trig_bit2 && Ls2_pad[0]==11 && Ls2_nthit==1 )
	     {
                RF_s2_mean = RF_s2_pad11_meanCorr;
	     }
	   else if( LHRS_trig_bit2 && (Ls2_pad[0]==10 && Ls2_pad[1]==11 ))
