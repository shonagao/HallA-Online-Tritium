...
void L_add6_5()
{
  ...
  //Int_t runNUM = 90854;
  // Int_t evtNUM = 10000;
TH1F *h5=new TH1F("h5","h5 with  + 9.835e-10",500,0.10e-6,0.15e-6);
 TString filename;
 for(int irun=100400;irun<100414;irun++)
   {
...
RF_s2_pad5_meanCorr =(Ls2_tdchit[37] -Ls2_tdchit[47])*tdcTime  - (((Ls2_tdchit[30]- Ls2_tdchit[5])*tdcTime + (Ls2_tdchit[37] - Ls2_tdchit[53])*tdcTime )/2.0 + 7.88134e-09*L_trx[0]  -4.07972e-08*L_trth[0] -1.24632e-12*Ls2_ladc[5] + 9.835e-10); 
...
// Do not change any variables
 ...
 // Defining cuts
 if(LHRS_trig_bit2 && Ls2_pad[0]==5 && Ls2_nthit==1)
	   {
             RF_s2_mean = RF_s2_pad5_meanCorr;
	   }
	   else if( LHRS_trig_bit2 && Ls2_pad[0]==6 && Ls2_nthit==1 )
	     {
               RF_s2_mean = RF_s2_pad6_meanCorr;
	     }
	   else if( LHRS_trig_bit2 && (Ls2_pad[0]==5 && Ls2_pad[1]==6 ))
