// Author  Bishnu
//06/13/2018/
// looking for an  event that either pass through pad#3 or pad# 2 or both
...
void coinTime3_2()
{
 ...
 TH1F *h2=new TH1F("h2","#2 + 2.0975;RF -s2 time in sec;counts",500,0.55e-6,0.6e-6);
 TString filename;
 for(int irun=90849;irun<90858;irun++)
   {
     if(irun==90851 || irun ==90852 || irun==90853) continue;
  ...
  for(Long64_t i=0;i<nentries;i++)
    {
      ...
      RF_s2_pad2_meanCorr = (Rs2_tdchit[15]-Rs2_tdchit[14])*tdcTime -(((Rs2_tdchit[15]-Rs2_tdchit[18])*tdcTime + (Rs2_tdchit[46]-Rs2_tdchit[50])*tdcTime)/2.0 + 4.91939e-09*R_trx[0] -4.46911e-08*R_trth[0] -8.66369e-13*Rs2_lac[2] + 2.0975e-9 );



RF_s2_pad3_meanCorr = (Rs2_tdchit[15] - Rs2_tdchit[14])*tdcTime -(((Rs2_tdchit[15]-Rs2_tdchit[19])*tdcTime + (Rs2_tdchit[46]-Rs2_tdchit[51])*tdcTime)/2.0 +5.41034e-09 *R_trx[0] -4.79507e-08*R_trth[0] - 3.84824e-13*Rs2_lac[3] + 2.1e-9); 
// 2.1 e-9 gives , 336,353 and 360 ps
			      ...
			      if(HRS_trig_bit5 && Rs2_pads[0]==2 && Rs2_nthit==1)
				{
				  RF_s2_mean =  RF_s2_pad2_meanCorr;
				}
			     else if( HRS_trig_bit5 && Rs2_pads[0]==3 && Rs2_nthit==1 )
				{
                                RF_s2_mean =  RF_s2_pad3_meanCorr;
				}
			     else if( HRS_trig_bit5 && Rs2_pads[0]==2 && Rs2_pads[1]==3 )
			       {

                                 RF_s2_mean =(RF_s2_pad2_meanCorr + RF_s2_pad3_meanCorr)/2.0;
