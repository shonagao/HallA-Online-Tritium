...
// in this code I am going to learn the graphical cuts
#include "TH1F.h"
#include "TCanvas.h"
#include "TString.h"
#include "TFile.h"
...
// correcting for X and X' again//////07/24/2018
...
void RHRS_cuts()
{
  gStyle->SetOptStat(1111);
   Double_t tdcTime = 56.23e-12; // this is the conversion per chanel ie 56.23 picosec
    TH2F *h1 = new TH2F("h1", "",700,0.30e-6,0.35e-6,16,0,16); //For Two Dimensional Histogram
    // TH1F *h1 = new TH1F("h1", "",500,0.30e-6,0.35e-6);// one dimensional Histogram
   //  TProfile *p1 = new TProfile("p1", "",200,-1,1,0.3190e-6,0.3221e-6);
  TString filename;
  for(int irun= 100400;irun<100402;irun++)
    {
 ...
 /*
 Double_t c1 = -2.20028e-12;
 for(int i = 0;i<14;i++)
   {
     corr_adc[i] = corr_adc[i] + c1;
   }
*/
...
// #some calculation depending on TDC information and correction depending on i ([paddle which was hit)
	  //  h1->Fill(RF_s2_mean,R_trx[0]); // for two dimensional histogram
...
////////////////////////////////////////////////////////
  ...
  l1.DrawLatex(0.575e-6,700,Form("#color[2]{#sigma #approx 395 ps}"));
   ...
   TLine *l2= new TLine(0.325e-6,-50,0.325e-6,50);
