...
for( int i=2;i<16;i++)
      ...
      if( HRS_trig>1 && Rs2_nthit==1 && Rs2_pads[0]==i &&  R_trbeta>0.76 && R_trbeta<0.80  ) // R.tr.beta>0.76 && R.tr.beta<80
	{
	  ...
	  h1->Fill(RF_s2_mean,i);// for 2 Dimensional;
          h1->Fill(RF_s2_mean);// For opne  dimensional
