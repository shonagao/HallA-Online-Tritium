...
///////////////////////////////////////////////////////////////////
// The code is corrected for ee'p data from marathan data
//here I just copied from RHRStime_3.c. This time I am using optics data from short range experiment. Probably will correct RF -s2 with respect to X or X'
// September 29, 2018
#include "TH1F.h"
#include "TCanvas.h"
#include "TString.h"
...
// looking for the short range data on October 1 2018. copied from RHRStime_3.C
// This based on the new  channel map for the F1TDC's ( updated on September 24,2018)
Int_t NEvent = 1000;
...
void RHRStime_4()
{
  gStyle->SetOptStat(111111);
   ...
   // TH2F *h1 = new TH2F("h1", "",500,0.3e-6,0.35e-6,14,0.5,14.5);// two dimensional Histogram
   TH1F *h1 = new TH1F("h1", "",500,0.3e-6,0.35e-6);// one dimensional Histogram
   
  TString filename;
  for(int irun= 93498;irun<93510;irun++)  // ee'p 100125,100645
    { // short range 93498 to  93519
...
for( int i=2;i<16;i++)
      ...
      if( HRS_trig>1 && Rs2_nthit==1 && Rs2_pads[0]==i &&  R_trbeta>0.76 && R_trbeta<0.80  ) // R.tr.beta>0.76 && R.tr.beta<80
	{
	  ...
	  //  double RF_s2_mean = (Rs2_tdchit[15] - Rs2_tdchit[14]) * tdcTime  - (((Rs2_tdchit[15]-Rs2_tdchit[i+16])*tdcTime + (Rs2_tdchit[46]-Rs2_tdchit[i+48])*tdcTime)/2.0 + corr_x[i-2]*R_trx[0] + corr_th[i-2]*R_trth[0] + corr_adc[i-2]*Rs2_lac[i] + alignment[i-2]); // for ee'p data
	    
double RF_s2_mean = (Rs2_tdchit[9] - Rs2_tdchit[15]) * tdcTime  - (((Rs2_tdchit[9]-Rs2_tdchit[i+16])*tdcTime + (Rs2_tdchit[46]-Rs2_tdchit[i+48])*tdcTime)/2.0 + corr_x[i-2]*R_trx[0] + corr_th[i-2]*R_trth[0] + corr_adc[i-2]*Rs2_lac[i] + alignment[i-2]); // for short range data
