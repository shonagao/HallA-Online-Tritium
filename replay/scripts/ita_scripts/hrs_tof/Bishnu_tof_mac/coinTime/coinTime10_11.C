// Author  and Bishnu
//06/21/2018/
// looking for an  event that either pass through pad#10 or pad# 11 or both
...
void coinTime10_11()
{
 ...
 TH1F *h10=new TH1F("h10","h10",500,0.55e-6,0.6e-6); 
...
RF_s2_pad10_meanCorr = (Rs2_tdchit[15] - Rs2_tdchit[14]) * tdcTime -(((Rs2_tdchit[15]-Rs2_tdchit[26])*tdcTime + (Rs2_tdchit[46]-Rs2_tdchit[58])*tdcTime)/2.0 + 7.5127e-9*R_trx[0]  - 3.59703e-08*R_trth[0] - 1.02612e-12*Rs2_lac[10] + 1.0e-10);
    

       RF_s2_pad11_meanCorr = (Rs2_tdchit[15] - Rs2_tdchit[14]) * tdcTime -(((Rs2_tdchit[15]-Rs2_tdchit[27])*tdcTime + (Rs2_tdchit[46]-Rs2_tdchit[59])*tdcTime)/2.0 + 8.48865e-9*R_trx[0]  -3.76206e-08*R_trth[0] - 1.87664e-12*Rs2_lac[11]+ 2.2e-10 ); 
 ...
 //	h8->Fill(RF_s2_pad8_meanCorr);
	...
	//Defining cuts 
	if( HRS_trg_bit5 && Rs2_pad[0]==10 &&  Rs2_nthit==1)
	  {
	    RF_s2_mean = RF_s2_pad10_meanCorr;
	  }
	else if( HRS_trg_bit5 && Rs2_pad[0]==11 && Rs2_nthit==1 )
	  {
               RF_s2_mean = RF_s2_pad11_meanCorr;
	  }

	else if(  HRS_trg_bit5 &&((Rs2_pad[0]==10 && Rs2_pad[1]==11) /*||(Rs2_pad[0]==7 || Rs2_pad[0]==8)*/))
	{
          RF_s2_mean = (RF_s2_pad10_meanCorr + RF_s2_pad11_meanCorr)/2.0;
	}
	else 
	{
	  RF_s2_mean = 0;
