// Author  and Bishnu
//06/21/2018/
// looking for an  event that either pass through pad#12 or pad# 13 or both
...
void coinTime12_13()
{
 ...
 TH1F *h12=new TH1F("h12","h12",500,0.55e-6,0.6e-6); 
  ...
  for (int irun = 90854; irun < 90855; irun++)
    {
...
RF_s2_pad12_meanCorr = (Rs2_tdchit[15] - Rs2_tdchit[14]) * tdcTime -(((Rs2_tdchit[15]-Rs2_tdchit[28])*tdcTime + (Rs2_tdchit[46]-Rs2_tdchit[60])*tdcTime)/2.0 + 5.42156e-9*R_trx[0]  - 3.68166e-08*R_trth[0] - 3.19282e-12*Rs2_lac[12] + 2.20e-9);

   RF_s2_pad13_meanCorr = (Rs2_tdchit[15] - Rs2_tdchit[14]) * tdcTime -(((Rs2_tdchit[15]-Rs2_tdchit[29])*tdcTime + (Rs2_tdchit[46]-Rs2_tdchit[61])*tdcTime)/2.0 + 6.27864e-9*R_trx[0]  - 3.51979e-08*R_trth[0] - 3.37812e-12*Rs2_lac[13] + 2.0e-9); 
 ...
 //	h8->Fill(RF_s2_pad8_meanCorr);
	...
	//Defining cuts 
	if( HRS_trg_bit5 && Rs2_pad[0]==12 &&  Rs2_nthit==1)
	  {
	    RF_s2_mean = RF_s2_pad12_meanCorr;
	  }
	else if( HRS_trg_bit5 && Rs2_pad[0]==13 && Rs2_nthit==1 )
	  {
               RF_s2_mean = RF_s2_pad13_meanCorr;
	  }

	else if(  HRS_trg_bit5 &&((Rs2_pad[0]==12 && Rs2_pad[1]==13) /*||(Rs2_pad[0]==7 || Rs2_pad[0]==8)*/))
	{
