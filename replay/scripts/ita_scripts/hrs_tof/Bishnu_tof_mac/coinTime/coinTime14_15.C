// Author  and Bishnu
//06/22/2018/
// looking for an  event that either pass through pad#14 or pad# 15 or both
...
void coinTime14_15()
{
 ...
 TH1F *h14=new TH1F("h14","h15",500,0.55e-6,0.6e-6); 
...
RF_s2_pad14_meanCorr = (Rs2_tdchit[15] - Rs2_tdchit[14]) * tdcTime -(((Rs2_tdchit[15]-Rs2_tdchit[30])*tdcTime + (Rs2_tdchit[46]-Rs2_tdchit[62])*tdcTime)/2.0 + 5.78027e-9*R_trx[0]  - 3.54868e-08*R_trth[0] - 7.80362e-13*Rs2_lac[14] + 1.6e-9);

 
RF_s2_pad15_meanCorr = (Rs2_tdchit[15] - Rs2_tdchit[14]) * tdcTime -(((Rs2_tdchit[15]-Rs2_tdchit[31])*tdcTime + (Rs2_tdchit[46]-Rs2_tdchit[63])*tdcTime)/2.0 + 8.605971e-10*R_trx[0]  - 1.56998e-08*R_trth[0] - 7.65658e-13*Rs2_lac[15]+ 1.615e-9 ); 
 ...
 //	h8->Fill(RF_s2_pad8_meanCorr);
	...
	//Defining cuts 
	if( HRS_trg_bit5 && Rs2_pad[0]==14 &&  Rs2_nthit==1)
	  {
	    RF_s2_mean = RF_s2_pad14_meanCorr;
	  }
	else if( HRS_trg_bit5 && Rs2_pad[0]==15 && Rs2_nthit==1 )
	  {
               RF_s2_mean = RF_s2_pad15_meanCorr;
	  }

	else if(  HRS_trg_bit5 &&((Rs2_pad[0]==14 && Rs2_pad[1]==15) /*||(Rs2_pad[0]==7 || Rs2_pad[0]==8)*/))
	{
          RF_s2_mean = (RF_s2_pad14_meanCorr + RF_s2_pad15_meanCorr)/2.0;
	}
	else 
	{
	  RF_s2_mean = 0;
