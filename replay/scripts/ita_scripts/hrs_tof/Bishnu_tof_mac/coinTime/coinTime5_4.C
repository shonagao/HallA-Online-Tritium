...
void coinTime5_4()
{
 ...
 TH1F *h4=new TH1F("h4","either#4 hit or #5 or both;RF -s2 time in sec;counts",500,0.55e-6,0.6e-6);
 TString filename;
...
RF_s2_pad5_meanCorr = (Rs2_tdchit[15] - Rs2_tdchit[14])*tdcTime -(((Rs2_tdchit[15]-Rs2_tdchit[21])*tdcTime + (Rs2_tdchit[46]-Rs2_tdchit[53])*tdcTime)/2.0 + 9.22121e-09 *R_trx[0] -3.04303e-08*R_trth[0] - 12.08217e-13*Rs2_lac[5] +2.0e-9); 
			      ...
			      if(HRS_trig_bit5 && Rs2_pads[0]==4 && Rs2_nthit==1)
				{
				  RF_s2_mean =  RF_s2_pad4_meanCorr;
				}
			     else if( HRS_trig_bit5 && Rs2_pads[0]==5 && Rs2_nthit==1 )
				{
                                RF_s2_mean =  RF_s2_pad5_meanCorr;
				}
			     else if( HRS_trig_bit5 && Rs2_pads[0]==4 && Rs2_pads[1]==4 )
			       {

                                 RF_s2_mean =(RF_s2_pad4_meanCorr + RF_s2_pad5_meanCorr)/2.0;
