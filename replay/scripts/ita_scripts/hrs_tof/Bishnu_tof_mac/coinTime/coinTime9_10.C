// Author  and Bishnu
//06/21/2018/
// looking for an  event that either pass through pad#9 or pad# 10 or both
...
void coinTime9_10()
{
 ...
 TH1F *h9=new TH1F("h9","h9",500,0.55e-6,0.6e-6); 
  ...
  for(Long64_t i=0; i<nentries; i++)
    {
      ...
      UInt_t HRS_trg_bit5 = (static_cast<UInt_t>(HRS_trg)>>5)&1;
       ...
       RF_s2_pad9_meanCorr = (Rs2_tdchit[15] - Rs2_tdchit[14]) * tdcTime -(((Rs2_tdchit[15]-Rs2_tdchit[25])*tdcTime + (Rs2_tdchit[46]-Rs2_tdchit[57])*tdcTime)/2.0 + 7.54862e-09*R_trx[0]  -3.5057e-08*R_trth[0] - 1.01789e-12*Rs2_lac[9]+ 9.5e-10 ); 
...
RF_s2_pad10_meanCorr = (Rs2_tdchit[15] - Rs2_tdchit[14]) * tdcTime -(((Rs2_tdchit[15]-Rs2_tdchit[26])*tdcTime + (Rs2_tdchit[46]-Rs2_tdchit[58])*tdcTime)/2.0 + 7.5127e-9*R_trx[0]  - 3.59703e-08*R_trth[0] - 1.02612e-12*Rs2_lac[10] + 1.0e-10);
 ...
 //	h8->Fill(RF_s2_pad8_meanCorr);
	...
	//Defining cuts 
	if( HRS_trg_bit5 && Rs2_pad[0]==9 &&  Rs2_nthit==1)
	  {
	    RF_s2_mean = RF_s2_pad9_meanCorr;
	  }
	else if( HRS_trg_bit5 && Rs2_pad[0]==10 && Rs2_nthit==1 )
	  {
               RF_s2_mean = RF_s2_pad10_meanCorr;
	  }

	else if(  HRS_trg_bit5 &&((Rs2_pad[0]==9 && Rs2_pad[1]==10) /*||(Rs2_pad[0]==7 || Rs2_pad[0]==8)*/))
	{
          RF_s2_mean = (RF_s2_pad9_meanCorr + RF_s2_pad10_meanCorr)/2.0;
	}
	else 
	{
	  RF_s2_mean = 0;
