...
//-------- TTree data input ---------------//
 ...
 bool rarm;
 if(arm==0){rarm=true;}else{rarm=false;}
 ...
 //=================================================//
  ...
  bool trig_cut;
  bool tof_cut[chmax];
  double tof_cut_start[chmax],tof_cut_end[chmax];
  double sig[chmax];
  //======= TSlicesY Paramters =======//
 ...
 //====== Fill w/ Time-Walk Correction ======//
  ...
  double corr_s0r[chmax],corr_s0l[chmax];
...
ftof[i]=new TF1(Form("ftof[%d]",i),"gaus",min_tof[i],max_tof[i]);
 ...
 if(rarm){
  ...
  ffit_l[i]->SetParameter(0,-2.0e-8);   
  ffit_l[i]->SetParameter(1,-1.0-8);    
  ffit_l[i]->SetParLimits(0,-1.0e-8,-4.0e-7);   
 }else{
  ffit_r[i]->SetParameter(0,7.3e-8);   
  ffit_r[i]->SetParameter(1,-1.87e-8);   
