...
int max=10000; 
 ...
 htof_adc_l[i]=new TH2F(Form("htof_adc_l[%d]",i),Form("F1TDC TOF vs S2ch%d L-PMT ADC Hist",i),bin_tof,min_tof,max_tof,bin_adc,min_adc,max_adc);
    ...
    ctof_adc[i]=new TCanvas(Form("ctof_adc[%d]",i),Form("ctof_adc[%d]",i));
    ctof_adc[i]->Divide(1,2);
    // ctof_adc[i]=new TCanvas(Form("ctof_adc_l[%d]",i),Form("ctof_adc_l[%d]",i));
    ctof_pathl[i]=new TCanvas(Form("ctof_pathl[%d]",i),Form("ctof_pathl[%d]",i));
