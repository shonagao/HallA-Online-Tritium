...
void vdct0_Right(int run) {
    gSystem->Load("GmpVDCt0Calib_C.so");
    ...
    const TString ROOTfilePath = "/adaqfs/home/a-onl/tritium/replay/t2root/Rootfiles/";
    ...
    TString plotname = Form("R_vdct0_%d.pdf",run);
    GmpVDCt0Calib* vdct0 = new GmpVDCt0Calib;
    vdct0->Calibrate(T,0,"(DR.evtypebits>>5&1)",plotname);
    ...
    TH1F* hFrame = c1->DrawFrame(0,1200,GmpVDCt0Calib::GetNumberOfWiresPerPlane(),3000,"VDC t0 calibration result;Wire number;Time offset (TDC channel)");
    ...
    c1->SaveAs(Form("vdct0plot_%d.pdf",run));
}
