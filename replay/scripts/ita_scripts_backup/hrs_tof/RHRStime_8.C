...
// October 11, 2018 .. Florian already moved the origial position that is L1A=9 and Rf time = 15
// October 22 copied from RHRStime_5.c which is currently working with early short range data.
// October 23 copied from RHRStime_7.C to check the x theta variable as Florian said.
// Will be a two dimensional histogram
Int_t NEvent = 1000;
...
void RHRStime_8()
{
  gStyle->SetOptStat(1111);
   Double_t tdcTime = 56.23e-12; // this is the conversion per chanel ie 56.23 picosec
    ...
    TH2F *htof_pathl = new TH2F("htof_pathl", "",2000,0,300,500,0.5e-6,0.52e-6); //For Two Dimensional Histogram
   ...
   // TH1F *h1 = new TH1F("h1", "",1000,0.65e-6,0.7e-6);// one dimensional Histogram
    ...
    TProfile *p2 = new TProfile("h2", "",100,-1,1,0.5093e-6,0.5113e-6); //For Two Dimensional Histogram
  TString filename;
...
for( int i=2;i<16;i++)
  if(i==ch){
      ...
      if( HRS_trig>1 && Rs2_nthit==1 && Rs2_pads[0]==i &&  R_trbeta>0.76 && R_trbeta<1.01  ) // R.tr.beta>0.76 && R.tr.beta<80
	{
	  ...
	  //  double RF_s2_mean = (Rs2_tdchit[9] - Rs2_tdchit[15]) * tdcTime  - (((Rs2_tdchit[9]-Rs2_tdchit[i+16])*tdcTime + (Rs2_tdchit[46]-Rs2_tdchit[i+48])*tdcTime)/2.0 + corr_x[i-2]*R_trx[0] + corr_adc[i-2]*Rs2_lac[i] + alignment[i-2]); // late ST data
	    

	     double RF_s2_mean = (Rs2_tdchit[9] - Rs2_tdchit[15]) * tdcTime  - (((Rs2_tdchit[9]-Rs2_tdchit[i+16])*tdcTime + (Rs2_tdchit[46]-Rs2_tdchit[i+48])*tdcTime)/2.0 + corr_x[i-2]*R_trx[0] + corr_th[i-2]*R_trth[0] + corr_adc[i-2]*Rs2_lac[i] + alignment[i-2]); // late ST data
	    
	     pathl =pathl_rf[0]-pathl_s2[0];
	     if(j<100){
	       cout<<"pathl : "<<pathl<<endl;
	       cout<<"pathl_rf :"<<pathl_rf[0]<<endl;
	       cout<<"pathl_s2 :"<<pathl_s2[0]<<endl;
}
	  ...
	  htof_pathl->Fill(pathl,RF_s2_mean);
	  //	  T->Draw(Form("R.tr.th[0]:RF_s2_mean>>h1"));
